...
@push('css_or_js')
    <link href="{{asset('assets/back_end')}}/css/select2.min.css" rel="stylesheet"/>
    <link href="{{asset('assets/back_end/css/croppie.css')}}" rel="stylesheet">
                            ...
                            <form class="user" action="{{route('shop.apply')}}" method="post"
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <input type="text" class="form-control form-control-user" id="exampleFirstName"
                                               name="f_name" value="{{old('f_name')}}" placeholder="First Name"
                                               required>
                                    </div>
                                    <div class="col-sm-6">
                                        <input type="text" class="form-control form-control-user" id="exampleLastName"
                                               name="l_name" value="{{old('l_name')}}" placeholder="Last Name" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <div
                                            class="pass invalid-feedback">{{trans('messages.Repeat')}}  {{trans('messages.password')}} {{trans('messages.not match')}}
                                            .
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <div class="custom-file">
                                            ...
                                            <label class="custom-file-label"
                                                   for="customFileUpload">{{trans('messages.Upload')}} {{trans('messages.image')}}</label>
                                        </div>
                                    </div>
                                    <div class="col-sm-2" style="display: none">
                                        <center>
                                            <img
                                                style="width: auto;border: 1px solid; border-radius: 10px; max-height:100px;"
                                                id="viewer"
                                    ...
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <label for="name">{{ trans('messages.is_shipping_by_me') }}:</label>
                                        <label class="switch">
                                            <input type="checkbox" class="status" name="is_shipping_by_me" value="1">
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0 ">
                                        <select name="state_id" class="form-control" required
                                                onchange="getRequest('{{url('/')}}/seller/state/get-cities?state_id='+this.value,'city-select','select')">
                                            <option value="">{{ __('Select State') }}</option>
                                            @foreach (App\Model\Country::with('states')->get() as $country)
                                                <optgroup label="{{ $country->name }}">
                                                    @forelse ($country->states as $state)
                                                        <option
                                                            value="{{ $state->id }}" {{old('state_id')==$state->id? 'selected': ''}}>{{ $state->name }}</option>
                                                    @empty
                                                        <option disabled>{{ __('No States') }}</option>
                                                    @endforelse
                                                </optgroup>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-6">
                                        <select class="js-example-basic-multiple form-control"
                                                name="city_id" id="city-select">
                                            <option value="{{old('city_id')}}" selected disabled>---Select---</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="pb-1" style="display: none">
                                        <center>
                                            <img
                                    ...
                                    <div class="col-sm-6 mb-3 mb-sm-0 ">
                                        <div class="custom-file">
                                            <input type="file" name="logo" id="LogoUpload" class="custom-file-input"
                                                   accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                            <label class="custom-file-label"
                                                   for="LogoUpload">{{trans('messages.Upload')}} {{trans('messages.logo')}}</label>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="custom-file">
                                            ...
                                            <label class="custom-file-label"
                                                   for="BannerUpload">{{trans('messages.Upload')}} {{trans('messages.Banner')}}</label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button type="submit" class="btn btn-primary btn-user btn-block"
                                        id="apply">{{trans('messages.Apply')}} {{trans('messages.Shop')}} </button>
                            </form>
                            <hr>
                            <div class="text-center">
                                <a class="small" href="{{route('seller.auth.login')}}">Already have an account?
                                    Login!</a>
                            </div>
                        </div>
                    </div>
...
@push('script')
    ...
    <script src="{{asset('assets/back_end')}}/vendor/jquery/jquery.min.js"></script>
    <script src="{{asset('assets/back_end')}}/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="{{asset('assets/back_end')}}/vendor/jquery-easing/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="{{asset('assets/back_end')}}/js/sb-admin-2.min.js"></script>

    {{--Toastr--}}
    <script src={{asset("assets/back_end/js/toastr.js")}}></script>
    {!! Toastr::message() !!}
