...
<div class="card-body border-top p-9">
    ...
    <div class="row mb-6">
        <!--begin::Label-->
    {!! Form::label('parent_id', trans("messages.attributes.subcategory"), ['class' => 'col-lg-4 col-form-label fw-bold fs-6 required']) !!}
    <!--end::Label-->
        <!--begin::Col-->
        <div class="col-lg-8 fv-row ">
            <!--begin::Input-->
            {!! Form::select('parent_id', $subcategories, null, ['class' => 'form-select '.($errors->has('parent_id') ? 'is-invalid':'').' form-select-solid form-select-lg fw-bold',
'aria-label'=>trans('messages.select').' '.trans('messages.attributes.subcategory'), 'data-control'=>"select2", 'data-language' => app()->getLocale(), 'data-placeholder'=>trans('messages.select').' '.trans('messages.attributes.subcategory').'...', 'required']) !!}
...
'aria-label'=>trans('messages.select').' '.trans('messages.attributes.state'), 'data-control'=>"select2", 'data-language' => app()->getLocale(), 'data-placeholder'=>trans('messages.select').' '.trans('messages.attributes.state').'...', 'required']) !!}
    ...
    <!--end::Label-->
        <!--begin::Col-->
        <div class="col-lg-8 fv-row ">
            @include('layouts.commons.boolean-field', ['model' => !empty($service) ? $service : null, 'name' => 'is_active'])
        </div>
        <!--end::Col-->
    </div>
    <!--end::Input group-->
