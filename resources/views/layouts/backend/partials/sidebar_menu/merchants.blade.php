<div data-kt-menu-trigger="click"
     class="menu-item menu-accordion {{ Request::is($guard.'/merchants*') || Request::is($guard.'/merchant-branches*') || Request::is($guard.'/merchant-users*') ? 'here hover show' : '' }}">
											<span class="menu-link">
												<span class="menu-icon">

<!--begin::Svg Icon | path: assets/media/icons/duotune/medicine/med005.svg-->
<span class="svg-icon svg-icon-muted svg-icon-2"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
<path opacity="0.3" d="M17.9061 13H11.2061C11.2061 12.4 10.8061 12 10.2061 12C9.60605 12 9.20605 12.4 9.20605 13H6.50606L9.20605 8.40002V4C8.60605 4 8.20605 3.6 8.20605 3C8.20605 2.4 8.60605 2 9.20605 2H15.2061C15.8061 2 16.2061 2.4 16.2061 3C16.2061 3.6 15.8061 4 15.2061 4V8.40002L17.9061 13ZM13.2061 9C12.6061 9 12.2061 9.4 12.2061 10C12.2061 10.6 12.6061 11 13.2061 11C13.8061 11 14.2061 10.6 14.2061 10C14.2061 9.4 13.8061 9 13.2061 9Z" fill="black"></path>
<path d="M18.9061 22H5.40605C3.60605 22 2.40606 20 3.30606 18.4L6.40605 13H9.10605C9.10605 13.6 9.50605 14 10.106 14C10.706 14 11.106 13.6 11.106 13H17.8061L20.9061 18.4C21.9061 20 20.8061 22 18.9061 22ZM14.2061 15C13.1061 15 12.2061 15.9 12.2061 17C12.2061 18.1 13.1061 19 14.2061 19C15.3061 19 16.2061 18.1 16.2061 17C16.2061 15.9 15.3061 15 14.2061 15Z" fill="black"></path>
</svg></span>
                                                    <!--end::Svg Icon-->
										</span>
												<span class="menu-title">{{trans('messages.attributes.merchants')}}</span>
												<span class="menu-arrow"></span>
											</span>
    <div class="menu-sub menu-sub-accordion menu-active-bg">
        <div class="menu-item">
            <a class="menu-link {{ Request::is($guard.'/merchants*') ? 'active' : '' }}"
               href="{{route($guard.'.merchants.index')}}">
														<span class="menu-bullet">
															<span class="bullet bullet-dot"></span>
														</span>
                <span class="menu-title">{{trans('messages.attributes.merchants')}}</span>
            </a>
        </div>
        <div class="menu-item">
            <a class="menu-link {{ Request::is($guard.'/merchantBranches*') ? 'active' : '' }}"
               href="{{route($guard.'.merchantBranches.index')}}">
														<span class="menu-bullet">
															<span class="bullet bullet-dot"></span>
														</span>
                <span class="menu-title">{{trans('messages.attributes.merchant_branches')}}</span>
            </a>
        </div>
        <div class="menu-item">
            <a class="menu-link {{ Request::is($guard.'/merchantUsers*') ? 'active' : '' }}"
               href="{{route($guard.'.merchantUsers.index')}}">
														<span class="menu-bullet">
															<span class="bullet bullet-dot"></span>
														</span>
                <span class="menu-title">{{trans('messages.attributes.merchant_users')}}</span>
            </a>
        </div>
    </div>
</div>
